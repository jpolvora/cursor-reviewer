                ...
                if (isBot) {
                    if (!isResolved) {
                        ...
                        activeThreads.push({
                            ...
                            hasResolutionReply: comments.some((c) => c.databaseId !== firstComment.databaseId &&
...
`;
    ...
    async setPullRequestComments(botTag, reviewsJson, existingKeys, log) {
        ...
        const reviews = (reviewsObject.reviews ?? []).filter((review) => isPublishableReview(review, this.config.scoreMin));
    ...
    emitPipelineReviewOutput(gate, reviews, dryRun, tokenUsage, log = console.log) {
    ...
    buildGHSummaryMarkdown(gate, reviews, dryRun, tokenUsage) {
        const lines = [];
        ...
        if (tokenUsage) {
            lines.push(`- **Tokens total:** ${tokenUsage.totalTokens.toLocaleString()} (Input: ${tokenUsage.inputTokens.toLocaleString()}, Output: ${tokenUsage.outputTokens.toLocaleString()})`);
        }
        lines.push('');
        if (reviews.length > 0) {
