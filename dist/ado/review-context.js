import { commentHasBotTag, normalizeFilePath, stripHtml, } from './utils.js';
import { REVIEW_SUMMARY_MARKER, commentBodyHasResolutionReply, } from '../git/markers.js';
...
function threadHasResolutionReply(thread, botTag) {
    return thread.comments.some((comment) => !comment.isDeleted &&
...
export function filterGatePendingThreads(threads, botTag) {
...
export async function getPullRequestReviewContext(client, pullRequestId, botTag, log) {
    try {
        ...
        for (const thread of existingThreads.value) {
            ...
            const botComment = thread.comments.find((comment) => !comment.isDeleted && commentHasBotTag(comment.content, botTag));
            if (!botComment) {
                continue;
            }
