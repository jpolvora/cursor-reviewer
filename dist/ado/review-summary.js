...
function correctMisplacedWorkItemTitles(text, prId, prTitle, workItemTitles, formatPrRef) {
    if (prId <= 0 || !prTitle) {
        return text;
    }
    ...
    for (const wiTitle of workItemTitles) {
        ...
        const afterPrTitle = new RegExp(`(${escapeRegExp(prRef)})\\s*([«"']?)${escapeRegExp(wiTitle)}\\2`, 'i');
    ...
    if (startsWithPrRef.test(result) && !result.includes(prTitle)) {
        result = result.replace(startsWithPrRef, () => `Revisão somente leitura da ${prRef} ("${prTitle}")`);
    }
    return result;
}
function sanitizeForAdo(text, options) {
    ...
    for (const wiId of workItemIds) {
        result = result.replace(new RegExp(`\\b(?:WI|Work\\s*Item|User\\s*Story|Task|Bug)\\s*#\\s*${wiId}\\b`, 'gi'), `Work Item ${wiId}`);
...
function sanitizeForGithub(text, options) {
    const prId = options.pullRequestId;
    const prTitle = options.prTitle?.trim() ?? '';
    const workItemTitles = [...new Set((options.workItemTitles ?? []).map((t) => t.trim()).filter(Boolean))];
    let result = text;
 ...
 */
export function sanitizeReviewSummaryForPlatform(summaryText, options) {
