...
export function parseCodeReviewResponse(raw) {
    ...
    if (reviews.length < flattenedIncoming.length) {
        console.warn(`Policy: ${flattenedIncoming.length - reviews.length} review(s) descartado(s) — score ≤ 5, campos obrigatórios ausentes ou contrato inválido.`);
    }
...
export async function setPullRequestComments(client, pullRequestId, botTag, reviewsJson, existingKeys, log) {
    const posted = [];
...
export function getNewReviewsFromPlan(reviewsJson, existingKeys) {
    const reviewsObject = JSON.parse(reviewsJson);
