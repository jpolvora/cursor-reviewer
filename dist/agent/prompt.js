...
function buildExecutionContext(config, context) {
    ...
    lines.push('', '## Contexto da execução', '', `\`cwd\` = \`${config.repoRoot}\`. Diff e rules já estão embutidos abaixo; use tools para expandir contexto na Fase 2.`, '');
    const platformLabel = config.provider === 'github' ? 'GitHub' : 'Azure DevOps';
    if (config.pullRequestId > 0) {
        lines.push(`- **Pull Request ID (${platformLabel}):** #${config.pullRequestId}`, `- **Fonte do ID da PR:** \`${config.pullRequestIdSource || 'desconhecida'}\``, `- **Atenção (IDs):** não confunda o **ID da PR** com IDs de Work Items (User Story / Task / Bug) linkados.`, `- **Atenção (textos):** título/descrição da **PR** ≠ título/descrição de **Work Item / Task**. Ao citar o que a mudança faz (comentários ou \`reviewSummary\`), use **somente** a seção \`## Pull Request\` — nunca o texto de \`## Linked Work Items\`.`, '');
    }
    lines.push(`- **Branch:** \`${sourceRef}\` → \`${targetRef}\``, `- **Diff range:** \`${diffScopeLabel}\``, `- **Stack:** \`${config.stack}\``, `- **Arquivos elegíveis:** ${context.diffStats.fileCount}`, context.diffStats.files.length > 0
...
function buildScoreMinOverrideSection(scoreMin) {
    if (scoreMin === DEFAULT_SCORE_MIN)
        return [];
    return [
        '---',
        '',
...
function buildSeedTestSection(scoreMin) {
    return [
...
function buildReviewSummaryLinkPolicy(provider) {
    if (provider === 'github') {
...
function buildVerdictAndPlatformPolicy(provider) {
...
export function buildAgentPrompt(config, context) {
    ...
    if (stackPromptContent) {
        sections.push('---', '', `# Recomendações Específicas da Stack (${config.stack})`, '', stackPromptContent, '');
    }
