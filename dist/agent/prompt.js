...
function buildExecutionContext(config, context) {
    ...
    if (config.pullRequestId > 0) {
        lines.push(`- **Pull Request ID (Azure DevOps):** #${config.pullRequestId}`, `- **Fonte do ID da PR:** \`${config.pullRequestIdSource || 'desconhecida'}\``, `- **Atenção:** não confunda o ID da PR com IDs de Work Items (User Story/Task) linkados à PR.`, '');
    }
...
function buildTwoPhaseWorkflow(context) {
    const diffRange = context.gitContext.diffRange;
...
function buildVerdictAndAdoPolicy() {
    return [
        '',
        '### Veredito final',
        '',
