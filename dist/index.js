...
import { getCodeReviewPostingPlan, getNewReviewsFromPlan, parseCodeReviewResponse, simulateThreadResolution, } from './ado/post-comments.js';
...
import { formatDiffSizeKb, getDiffBreakdown, getDiffFileSummaries, prepareLocalReviewWorkspace, } from './git/diff.js';
...
async function main() {
    ...
    const wouldPostReviewsPre = getNewReviewsFromPlan(getCodeReviewPostingPlan(parsed, gatePendingBeforePost.length > 0).reviewsJson, reviewContext.existingKeys, config.scoreMin);
    ...
    const wouldPostReviews = escalate
        ? getNewReviewsFromPlan(getCodeReviewPostingPlan(effectiveParsed, gatePendingBeforePost.length > 0).reviewsJson, reviewContext.existingKeys, config.scoreMin)
        : wouldPostReviewsPre;
    ...
    if (isDryRunOrNoContext) {
        ...
        if (hasContext) {
            const simulated = simulateThreadResolution(reviewContext.activeThreads, pendingThreads, parsed.resolvedThreads);
        ...
        resolvedCount = await provider.resolvePullRequestReviewThreads(config.botTag, reviewContext.activeThreads, parsed.resolvedThreads, (msg) => logger.info(msg));
        ...
        if (postingPlan.postSummary && !gateBeforeSummary.shouldFail) {
            ...
            await provider.setPullRequestReviewSummary(config.botTag, postingPlan.reviewSummary, reviewContext.allThreads, (msg) => logger.info(msg), prTitle, workItemSummaries);
        }
    ...
    console.log(formatGateSummary(gate, agentResult.agentId, agentResult.runId, isDryRunOrNoContext, agentResult.tokenUsage));
