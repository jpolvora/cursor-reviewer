...
async function main() {
    ...
    const wouldPostReviewsPre = getNewReviewsFromPlan(getCodeReviewPostingPlan(parsed, gatePendingBeforePost.length > 0).reviewsJson, reviewContext.existingKeys);
    ...
    const wouldPostReviews = escalate
        ? getNewReviewsFromPlan(getCodeReviewPostingPlan(effectiveParsed, gatePendingBeforePost.length > 0).reviewsJson, reviewContext.existingKeys)
        : wouldPostReviewsPre;
    ...
    if (isDryRunOrNoContext) {
        ...
        if (postingPlan.postSummary && !gateBeforeSummary.shouldFail) {
            ...
            await provider.setPullRequestReviewSummary(config.botTag, postingPlan.reviewSummary, reviewContext.allThreads, (msg) => logger.info(msg));
        }
